...
private val HEADER_DECLARATION = MarkerType(
        ...
        object : LineMarkerNavigator() {
            override fun browse(e: MouseEvent?, element: PsiElement?) {
                element?.let { navigateToHeaderDeclaration(e, it.parent as KtDeclaration) }
            }
        }
)
...
private fun collectImplementationMarkers(declaration: KtNamedDeclaration,
    ...
    if (commonModuleDescriptor.allImplementingModules.all {
...
private fun collectHeaderMarkers(declaration: KtNamedDeclaration,
    ...
    if (!headerModuleDescriptor.hasDeclarationOf(descriptor)) return
